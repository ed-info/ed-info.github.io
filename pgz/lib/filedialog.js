...
(function() {
  ...
  the view.
   */
  changeFolder = async function(destination) {
    await fsToBrowse.cd(destination);    
    //tellPage(['changedFolder', await fsToBrowse.getCwd()]);
...
window.buttonClicked = async function(name) {
  let args = [];

  // === Обробка "New folder" ===
  if (name === 'New folder') {
    const folderName = prompt('Введіть назву створюваної теки', 'Мої файли');
    if (!folderName) return;

    try {
      const success = await fsToBrowse.mkdir(folderName.trim());
      if (success) {
        await updateFileBrowser();
      } else {
        alert('Тека з таким іменем вже існує.');
      }
    } catch (err) {
      console.error('Помилка створення теки:', err);
  ...
  // === Обробка "Save" ===
  if (name === 'Save') {
    const path = fsToBrowse.getCwd();
    const sep = FileSystem.prototype.pathSeparator;
    const fullPath = (path.endsWith(sep) ? path : path + sep) + saveFileName.value;

    if (window.fileBeingMoved.name) {
      // Виконуємо переміщення або копіювання
      let success;
      if (window.fileBeingMoved.copy) {
        success = await fsToBrowse.cp(window.fileBeingMoved.full, fullPath);
        name = success ? 'Copied' : 'Copy failed';
      } else {
        success = await fsToBrowse.mv(window.fileBeingMoved.full, fullPath);
        name = success ? 'Moved' : 'Move failed';
      }
    } else {
      // Звичайне збереження — передаємо шлях
      args.push(fullPath);
    }
  }
  ...
  // === Обробка "Open" ===
  if (name === 'Open') {
    if (!window.fileToBeOpened) {
      alert('Будь ласка, спочатку виберіть файл.');
      return;
    }
    const path = fsToBrowse.getCwd();
    const sep = FileSystem.prototype.pathSeparator;
    const fullPath = (path.endsWith(sep) ? path : path + sep) + window.fileToBeOpened;
    args.push(fullPath);
    try {
      await viewFile(fullPath);
    } catch (err) {
      console.error('Помилка перегляду файлу:', err);
      await message('Помилка', 'Не вдалося відкрити файл.');
    }
  }

  // === Обробка "Open this folder" → "Open folder" ===
  if (name === 'Open this folder') {
    args.push(fsToBrowse.getCwd());
    name = 'Open folder';
  }

  // === Обробка "Get file" (завантаження з комп’ютера) ===
  if (name === 'Get file') {
    // Ця дія вже обробляється через input#file-btn у головному документі,
    // тому тут можна просто повернути або викликати зовнішню функцію.
    // Якщо потрібно — викличіть зовнішню функцію через postMessage:
    tellPage(['buttonClicked', 'Get file']);
    window.fileBeingMoved = {};
    selectFile(null);
  ...
  // === Надсилання результату батьківському вікну ===
   ...
   */
askToDeleteEntry = async function(entry) {
  if (confirm('Бажаєте видалити назавжди "' + entry + '"?')) {
    try {
      await fsToBrowse.rm(entry);
      await updateFileBrowser(); // також await, бо updateFileBrowser — async
    } catch (err) {
      console.error('Помилка видалення:', err);
      alert('Не вдалося видалити файл або теку.');
    }
  }
};
  /*
   ...
   */
  window.updateFileBrowser = async function() {
...
*/   
    ref = await fsToBrowse.ls('.', 'folders');
...
*/
   ...
   */
    ref1 = await fsToBrowse.ls('.', 'files');
...
if (features.viewFiles) {
  (function(file) {
    return X += makeActionLink(icon('view'), 'Переглянути файл ' + file, async function() {
      const path = fsToBrowse.getCwd();
      const sep = FileSystem.prototype.pathSeparator;
      const fullPath = (path.endsWith(sep) ? path : path + sep) + file;
      try {
        await viewFile(fullPath);
      } catch (err) {
        console.error('Помилка перегляду:', err);
   ...
   */    
    for (index = l = 0, len2 = buttons.length; l < len2; index = ++l) {
      ...
      var onclick = vimg+" style='width:130px;height:32px;font-size:10' onclick='buttonClicked(\"" + text + "\");'";
      
      
      buttons[index] = buttons[index] + vtxt  + "' id='statusBarButton" + text + "' " + disable + onclick + ">";
      if (text === "New folder") { 
		   buttons[index]="<button style='width:143px;height:32px;font-size:10' onclick='newFolder()'"+"><i class='fa fa-folder' ></i> Створити теку</button>"
		  };
      if (text === "Get file") { 
		   buttons[index]="<button style='width:133px;height:32px;font-size:10' onclick='document.getElementById("+ String.fromCharCode(34)+'getfile'+ String.fromCharCode(34)+").click()'"+"><i class='fa fa-file-o' ></i> Додати файл</button><input type='file' id='getfile' onchange='readFile(this)' style='display:none'>"
		  };
...
async function viewFile(file) {
  if (file === "/null") return;
  try {
    const content = await fsToBrowse.read(file);
    const ext = file.split('.').pop().toLowerCase();
    if (ext === 'txt') {
      openDialog(file, "<p>" + escapeHtml(content) + "</p>");
    } else if (['png', 'jpg', 'jpeg', 'gif'].includes(ext)) {
      openDialog(file, `<img src="${content}" style="max-width:100%">`);
    } else {
      openDialog(file, "Непідтримуваний формат");
    }
  } catch (err) {
    console.error('Помилка читання файлу:', err);
    message('Помилка', 'Не вдалося прочитати файл');
  }
}
   function newFolder(){      
