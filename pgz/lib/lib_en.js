...
var PythonIDE = {
	...
	debugHandler: function(result) {
		...
		if(PythonIDE.breakpoints[filename] && PythonIDE.breakpoints[filename][lineNumber] && PythonIDE.continueRunning) {
			PythonIDE.showHint("Paused at breakpoint on line " + (lineNumber));
			$('#btn_showREPL').show();
			breakpoint = true;
...
//		console.log(PythonIDE.runMode, breakpoint);
		...
		if(true || PythonIDE.runMode == "step" || breakpoint) {
			...
			while(context && context.$tmps && context.$lineno) {
				...
				html += '<h4>Local Variables in row ' + context.$lineno + '</h4><table><tr><th>Name</th><th>Data type</th><th>Value</th></tr>';
...
//---------------
	...
	welcomeMessage: "Code always, everywhere - for learning and for fun!",
			...
			if(code == undefined) {
				...
				while(r.$isSuspension) {
					...
					if(now - startTime > 5000) {
						PythonIDE.showHint("Stopped after 5 seconds to prevent browser malfunction");
						break;
					}
				} 
	...
	recover: function() {
        ...
        var html = '<p>A copy of your code is saved in your browser\'s storage every time you run it.</p>';
        html += '<p>Number of code backups currently stored in the vault: ' + PythonIDE.vault.length + '</p>';
        html += '<div id="slider_recover"></div><span id="recover_time">Drag the slider to view a previous version of the code</span>';

        html += '<p><button id="btn_recover">Recover the selected code copy</button></p>';
        html += '<p>If needed, you can delete all code copies stored in the recovery vault.</p><p><i class="fa fa-warning"></i> Be careful: once you click Delete All, this action cannot be undone.</p>';
        html += '<p><button id="btn_recover_clear" style="background-color:#FF0000;color:#FFFFFF">Delete all from storage</button></p>';
		...
		$('#slider_recover').slider({
			...
			slide: function(event, ui) {
				...
				$('#recover_time').html('<p class="recover_date">' + days[d.getDay()] + ' ' + d.getDate() + ' ' + months[d.getMonth()] + ' ' + d.getFullYear() + '</p><p class="recover_label">Saved at:</p><span id="recover_time_hours">' + (pad2(hours)) + '</span><span id="recover_time_mins">' + pad2(d.getMinutes()) + '</span>');
...
//----------------------------------------------------------------------------------
	...
	runCode: function(runMode) {
		...
		PythonIDE.showHint("The program is running...");
		...
		Sk.misceval.callsimAsync(handlers, function() {
			...
			PythonIDE.showHint('The program has finished executing');
			PythonIDE.continueRunning = false;
			if(PythonIDE.continueDebug)
				delete PythonIDE.continueDebug;
...
//------------------------------
		...
		if(PythonIDE.hash && !PythonIDE.forceReset && PythonIDE.aT && PythonIDE.aT[PythonIDE.hash] && PythonIDE.aT[PythonIDE.hash].c) {
			...
			if(JSON.stringify(PythonIDE.aT[PythonIDE.hash].c) != PythonIDE.defaultSavedCode) {
                ...
                PythonIDE.showHint("Displaying the code you last edited " + timeSince(PythonIDE.aT[PythonIDE.hash].t) + " ago");
...
//
	...
	resetFiles: function(all) {
		if(PythonIDE.aT && PythonIDE.hash && PythonIDE.aT[PythonIDE.hash] && PythonIDE.defaultSavedCode) {
			...
			PythonIDE.editor.setValue(PythonIDE.files[PythonIDE.currentFile]);
			PythonIDE.updateFileTabs();
			PythonIDE.showHint("Restore the default saved code. To undo, use the code recovery button.");
		}
	}
}
