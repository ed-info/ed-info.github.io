...
//---------------
    ...
    ? "\nWelcome to PGZ Studio"
    : "\nВітаємо у PGZ Studio",
...
//----------------------------------------------------------------------------------
	...
	runCode: function(runMode) {
        ...
        document.getElementById("btn_stopRunning").style.display = "block";
		...
		Sk.misceval.callsimAsync(handlers, function() {
            ...
            document.getElementById("btn_stopRunning").style.display = "none";
		...
		$('#radio_code_style').buttonset().change(function(e) {
			...
			switch(id) {
				...
				case 'radio_code_style_dusk':
					...
					$('#editorContainer').css({'background-color': '#002240'});
		...
		$('#radio_output_style').buttonset().change(function(e) {
			...
			switch(id) {
				case 'radio_output_style_light':
					$('#consoleContainer').css({'background-color': '#FFF','color':'#000'});
					PythonIDE.setOption('output_style', 'light')
				break;
				case 'radio_output_style_dark':
					$('#consoleContainer').css({'background-color': '#111','color':'#CCC'});
					PythonIDE.setOption('output_style', 'dark')
				break;
				case 'radio_output_style_dusk':
					$('#consoleContainer').css({'background-color': '#002240','color':'#CCC'});
        ...
        .click(function (e) {
            switch (e.currentTarget.id) {
                ...
                case 'btn_tools':
                    ...
                    $('.toolButton').toggleClass('visibleButton', toolsVisible);
                    break;
        
                case 'btn_show_output':
                    $('#dlg').dialog(
                        $('#dlg').dialog("isOpen") ? "close" : "open"
                    );
                    break;
                ...
                case 'btn_run':
                    const img = $("#btn_run").attr("src");
                    if (img === "./media/play.png") PythonIDE.runCode();
                    if (img === "./media/step.png") PythonIDE.runCode("step");
                    break;
            }
        });
