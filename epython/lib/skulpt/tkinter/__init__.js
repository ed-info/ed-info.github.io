...
//--------------        
        var commonDisplay = function(kwa, self, parent) {
            ...
            if(self.getHtml) {
                ...
                var e = $('#tkinter_' + self.id);
                if (props.fill) {
                    var f = Sk.ffi.remapToJs(props.fill);                    c
                    if (f==='x') {
                        e.css('width','100%');
                    } else if (f==='y') {
                        e.css('height','100%');
    ...
    function checkGeometryConflict(currentStyleId) { // перевірка на конфлікт менеджерів геометрії
        const styles = [
            { id: '#tkinter-pack-style', name: 'pack' },
            { id: '#tkinter-grid-style', name: 'grid' },
            { id: '#tkinter-place-style', name: 'place' },
        ];
...
var place = function(kwa, self) {
    ...
    if (anchor === 'center') {
        dy = -oHeight/2; dx = -oWidth/2; 
    }
    if (anchor === 'n') {
        dx = -oWidth/2; 
    } 
    if (anchor === 's') {
        dy = -oHeight; dx = -oWidth/2; 
    }       
    if (anchor === 'e') {
        dy = -oHeight/2; dx = -oWidth; 
    }
    if (anchor === 'w') {
        dy = -oHeight/2; 
    }
...
// pack layout manager ----------------------------------------
    var pack = function(kwa, self) {
        ...
        // Стилізація елемента
        const el = $('#' + elementId);
        el.css({ flex: '', width: 'auto', height: 'auto', margin: 0, padding: 0 });
...
// Color Chooser — tkinter.colorchooser.askcolor()
s.colorchooser = new Sk.misceval.buildClass(s, function($gbl, $loc) {
    $loc.askcolor = new Sk.builtin.func(function(defaultColor) {
...
s.Spinbox = new Sk.misceval.buildClass(s, function ($gbl, $loc) {
    ...
    const getHtml = function (self) {
        let minVal = 0, maxVal = 100, step = 1;
        let startVal = 0;
        self._values = [];
